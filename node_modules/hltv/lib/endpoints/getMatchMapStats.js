            ...
            switch (_d.label) {
                case 0:
                    ...
                    return [4, Promise.all([
                            mappers_1.fetchPage(config.hltvUrl + "/stats/matches/mapstatsid/" + id + "/-", config.loadPage),
                            mappers_1.fetchPage(config.hltvUrl + "/stats/matches/performance/mapstatsid/" + id + "/-", config.loadPage)
                        ])];
                case 1:
                    ...
                    map = mappers_1.getMapSlug(m$(m$('.match-info-box')
                        .contents()
                        .get(3))
                        .text()
                        .replace(/\n| /g, ''));
                    ...
                    roundHistory = rh1
                        .reduce(function (history, round, i) { return history.concat([round, rh2[i]]); }, [])
                    ...
                    playerOverviewStats = mappers_1.toArray(m$('.stats-table tbody tr')).map(function (rowEl) {
                        ...
                        return __assign({ id: id, name: rowEl.find('.st-player a').text(), kills: Number(rowEl
                                .find('.st-kills')
                                .contents()
                                .first()
                                .text()), hsKills: Number(rowEl
                                .find('.st-kills .gtSmartphone-only')
                                .text()
                                .replace(/\(|\)/g, '')), assists: Number(rowEl
                                .find('.st-assists')
                                .contents()
                                .first()
                                .text()), flashAssists: Number(rowEl
                                .find('.st-assists .gtSmartphone-only')
                                .text()
                                .replace(/\(|\)/g, '')), deaths: Number(rowEl.find('.st-deaths').text()), KAST: Number(rowEl
                                .find('.st-kdratio')
                                .text()
                                .replace('%', '')), killDeathsDifference: Number(rowEl.find('.st-kddiff').text()), ADR: Number(rowEl.find('.st-adr').text()), firstKillsDifference: Number(rowEl.find('.st-fkdiff').text()), rating: Number(rowEl.find('.st-rating').text()) }, performanceStats);
                    });
                    playerStats = {
                        team1: playerOverviewStats.slice(0, 5),
                        team2: playerOverviewStats.slice(5)
                    };
                    performanceOverview = mappers_1.toArray(p$('.overview-table tr'))
                        .slice(1)
                        .reduce(function (res, rowEl) {
                        var _a, _b;
                        ...
                        return {
                            team1: __assign({}, res.team1, (_a = {}, _a[property] = team1Stat, _a)),
                            team2: __assign({}, res.team2, (_b = {}, _b[property] = team2Stat, _b))
                        };
                    }, {});
                    return [2, {
